...
function Assert-CleanRepositoryState {
    ...
    $actualHead = (Get-GitOutput -Arguments @('rev-parse', 'HEAD') | Select-Object -First 1)
    if ($actualHead -ne $ExpectedHead) {
        throw 'Repository HEAD changed during P2-B3 closeout verification.'
    }
    $status = @(Get-GitOutput -Arguments @(
            'status', '--porcelain=v1', '--untracked-files=all'))
    if ($status.Count -ne 0) {
        throw 'P2-B3 closeout requires and must preserve a clean worktree.'
    }
}
...
function Invoke-VerificationStep {
    ...
    try {
        ...
        $timer.Stop()
        throw "P2-B3 closeout round $Round failed in ${Name}: $($_.Exception.Message)"
    }
    $timer.Stop()
    Write-Host "P2-B3 closeout round $Round ${Name}: SUCCESS ($($timer.Elapsed))"
}

function Assert-NoResidualResources {
    foreach ($containerPrefix in @('koiki-b34-', 'koiki-b35-')) {
        $containerNames = @(& docker ps `
        ...
        if ($containerNames.Count -ne 0) {
            throw "A P2-B3 verification container remains: $containerPrefix"
        }
    }
    ...
    foreach ($pattern in @(
        ...
        if ($temporaryDirectories.Count -ne 0) {
            throw "A P2-B3 temporary directory remains: $pattern"
        }
    }
...
$head = (Get-GitOutput -Arguments @('rev-parse', 'HEAD') | Select-Object -First 1)
...
Write-Host '=== P2-B3 final Root Reactor regression ==='
& $wrapper --batch-mode --no-transfer-progress clean verify
if ($LASTEXITCODE -ne 0) {
    throw "P2-B3 Root Reactor regression failed with exit code $LASTEXITCODE."
}

Write-Host '=== P2-B3 final Null Safety verification ==='
& $nullSafetyVerifier

Assert-CleanRepositoryState -ExpectedHead $head
Assert-NoResidualResources
Write-Host (
    'Phase 2 P2-B3 closeout succeeded: three consecutive T0-T6 aggregates, ' +
    'Root Reactor, Null Safety, inventory, sensitive-output and cleanup checks.')

...
$verificationRoot = Join-Path $temporaryRoot ("koiki-phase1b-cp3-" + [guid]::NewGuid().ToString('N'))
...
function Assert-SafeTemporaryPath {
    ...
    if ([System.IO.Path]::GetFileName($resolved) -notlike 'koiki-phase1b-cp3-*') {
        throw "Unexpected CP3 temporary directory name: $resolved"
    }
}
...
try {
    Invoke-KoikiMaven -Label 'Stage KOIKI CP3 release unit into isolated repository' -Arguments @(
    ...
    try {
        ...
        foreach ($requiredClass in @(
            'org/koikifw/starter/api/internal/KoikiApiAutoConfiguration.class',
            'org/koikifw/starter/api/internal/KoikiApiResilienceAutoConfiguration.class',
            'org/koikifw/starter/api/internal/KoikiProblemDetailsExceptionHandler.class'
        )) {
            if ($starterArchive.GetEntry($requiredClass) -eq $null) {
                throw "Required CP3 Starter implementation is missing: $requiredClass"
            }
        }
    ...
    if ($classEntries.Count -eq 0) {
        throw 'CP3 API Starter did not contain its internal auto-configuration implementation.'
    }
    if ($externalClassEntries.Count -ne 0) {
        $unexpectedClasses = $externalClassEntries.FullName -join ', '
        throw "CP3 introduced classes outside the internal package: $unexpectedClasses"
    }

    $inventory = Get-Content -Raw -LiteralPath $starterInventory
    if ($inventory -notmatch 'PUBLIC_JAVA_TYPES 0') {
        throw 'CP3 API Starter inventory does not declare PUBLIC_JAVA_TYPES 0.'
    }
    if ($inventory -notmatch 'PUBLIC_CONFIGURATION_PROPERTIES 6') {
        throw 'CP3 API Starter inventory does not declare its six configuration properties.'
    }
    if ($inventory -notmatch 'PROPERTY koiki[.]api[.]problem-details[.]enabled') {
        throw 'CP3 API Starter inventory does not declare the Problem Details switch.'
    }

    Invoke-KoikiMaven -Label 'Verify CP3 fine-grained API starter fixture' -Arguments @(
...
'@
    ...
    Invoke-KoikiMaven -Label 'Build CP3 Customer-like Runtime Consumer' -Arguments @(
        '-f', $consumerPom, 'clean', 'verify'
    )

    Invoke-KoikiMaven -Label 'Inspect CP3 Consumer runtime dependency boundary' -Arguments @(
        '-f', $consumerPom,
        '-pl', 'application', '-am',
        'dependency:tree',
        '-Dscope=runtime',
    ...
    foreach ($required in @(
        ...
        if ($runtimeDependencies -notmatch [regex]::Escape($required)) {
            throw "Required CP3 runtime dependency is missing: $required"
        }
    }
    ...
    Write-Host 'CP3 isolated Problem Details, Validation, JacksonException, Consumer HTTP, and dependency checks succeeded.'
} finally {
    Assert-SafeTemporaryPath -Path $verificationRoot

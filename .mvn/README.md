...
`bin`型は公式`maven-wrapper.jar`もRepositoryへ含めます。Wrapper 3.3.4の`only-script`型で確認された
PowerShellのsymlink判定不具合を回避するため、生成scriptへ独自patchを加えず`bin`型を採用しています。
現在の通常CIは`ubuntu-24.04`、開発者のlocal実行はWindows / PowerShellも検証対象です。OSごとに
別Wrapperや独自patchを持たず、同じMaven 3.9.16配布物を使用します。

...
- `.agents/skills/`と`.claude/skills/`には、正本を読むための薄いadapterだけを置く。
- ArchUnit、NullAway、Maven等が機械検査する規則はSkillへ複製しない。
- OpenSpec changeが存在する場合、そのproposal、spec、design、tasksを変更固有の正本とし、Skillで置き換えない。
- Walking Skeletonで未検証の規約を推測で固定しない。
- 変更で得た検証結果は`docs/architecture/validation/`へ記録する。

現在の2種はPhase 0で実証し、Phase 1〜3およびP4-ARでも正本として継続利用している。追加Skillは、
各Phaseで実証が必要となった横断的な判断手順に限定し、将来用途だけを理由に先行作成しない。
